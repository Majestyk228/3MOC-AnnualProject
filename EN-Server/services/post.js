...
async function getAllPosts() {
	const rows = await db.query("SELECT * FROM Post ORDER BY idPost DESC;", "");
	return rows;
}
...
async function getAllPostsFormatted(idCommunity) {
	...
	const request = "SELECT p.idPost, u.firstName, u.lastName, p.title, p.body, p.likes, p.dislikes FROM User u, Post p WHERE u.idUser = p.idUser AND p.idCommunity = " + idCommunity + " group by u.firstName, u.lastName, p.title, p.body, p.likes, p.dislikes ORDER BY p.idPost DESC; ";
...
async function getAllPostsFormattedAdmin(idCommunity) {
	//const request = "SELECT p.idPost, u.firstName, u.lastName, p.title, p.body, p.likes, p.dislikes, c.nbComment as comments, co.nbRewards as rewards FROM User u LEFT JOIN Post p ON " + idCommunity + " = p.idCommunity LEFT JOIN(select COUNT(c.idComment) as nbComment, idPost from Comment c) as c ON p.idPost = c.idPost left JOIN(select Count(co.idRewards) as nbRewards, idPost from Congratulate co) as co ON p.idPost = co.idPost left JOIN Associate a ON p.idCommunity = a.idCommunity where u.idUser = p.idUser group by u.firstName, u.lastName, p.title, p.body, p.likes, p.dislikes; ";
	const request = "SELECT p.idPost, a.firstName, a.lastName, p.title, p.body, p.likes, p.dislikes FROM Admin a, Post p WHERE a.idAdmin = p.idAdmin AND p.idCommunity = " + idCommunity + " group by a.firstName, a.lastName, p.title, p.body, p.likes, p.dislikes ORDER BY p.idPost DESC;";
	const rows = await db.query(request, "");
	return rows;
}
